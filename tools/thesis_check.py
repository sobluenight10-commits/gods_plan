...
for tk in HELD:
    ...
    elif tdr >= 2.5: verdict = "과잉반응 — 눌림매수 후보 (심리만 패닉)"
    elif tdr >= 1:   verdict = "정당한 반응 — 관망"
    else:            verdict = "⚠ 과소반응 — 시장이 피해를 덜 반영, 추가 하락 위험"
    out[tk] = {"status": verdict, "tdr": tdr, "P": d20, "R": round(R, 1),
               "worst": max(negs, key=lambda c: -c["c"])["title"][:60],
               "thesis": TM.get(tk, {}).get("thesis", ""), "kill": TM.get(tk, {}).get("kill", "")}
json.dump({"formula": "TDR = |20일낙폭%| / max(Σ 세그먼트비중×충격강도 ×100, 1) · ≥2.5 과잉반응 / 1~2.5 정당 / <1 과소반응",
